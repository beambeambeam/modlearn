...
import { DollarSign, Plus, Pencil, ArrowLeft, CheckCircle, X } from "lucide-react";
...
import {
  useContentPricing,
  useCreateContentPricing,
  useUpdateContentPricing,
} from "@/hooks/content/use-content-pricing";
import type { ContentPricingWindow } from "@/lib/types/commerce";
import { toast } from "sonner";

const EMPTY_FORM = { price: "", currency: "USD", effectiveFrom: "", effectiveTo: "" };

export default function ContentPricing({ contentId }: { contentId: string }) {
  const navigate = useNavigate();

  const { data, isLoading, isError } = useContentPricing(contentId);
  const createMutation = useCreateContentPricing(contentId);
  const updateMutation = useUpdateContentPricing(contentId);
  ...
  const handleEdit = (pw: ContentPricingWindow) => {
    setEditingId(pw.id);
    setForm({
      price:         pw.price,
  ...
  const handleSubmit = () => {
    ...
    if (editingId) {
      updateMutation.mutate(
        ...
        {
          onSuccess: () => { toast.success("Pricing updated"); handleCancel(); },
          onError:   () => toast.error("Failed to update pricing"),
        }
      );
    } else {
      createMutation.mutate(
        { contentId, price: form.price, currency: form.currency, effectiveFrom, effectiveTo },
        {
          onSuccess: () => { toast.success("Pricing created"); handleCancel(); },
    ...
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-0.5">
            Manage pricing windows for this content
          </p>
        </div>
        <Button
          type="button"
          variant="outline" className="gap-2"
          onClick={() => navigate({ to: "/admin/content/$id", params: { id: contentId } })}
        >
          <ArrowLeft size={16} />
          Back to Content
        </Button>
      ...
      <div className="grid grid-cols-[1fr_320px] gap-6 items-start">
        <div className="rounded-xl border bg-card">
          ...
          {windows.length === 0 ? (
            <div className="py-16 text-center text-sm text-muted-foreground">No pricing windows yet</div>
          ) : (
            windows.map((pw) => (
              <div key={pw.id} className="grid grid-cols-[1fr_80px_1fr_1fr_80px_auto] gap-4 items-center px-6 py-4 border-b last:border-0 hover:bg-muted/30">
                <span className="text-sm font-semibold">${pw.price}</span>
                <span className="text-sm text-muted-foreground">{pw.currency}</span>
                <span className="text-sm text-muted-foreground">{new Date(pw.effectiveFrom).toLocaleDateString()}</span>
                <span className="text-sm text-muted-foreground">{pw.effectiveTo ? new Date(pw.effectiveTo).toLocaleDateString() : "—"}</span>
                {pw.isActive
          ...
          {showForm && (
            <div className="rounded-xl border bg-card p-5 space-y-4">
              <div className="flex items-center justify-between">
                ...
                <Button type="button" variant="ghost" size="icon" onClick={handleCancel}><X size={16} /></Button>
              </div>
              <div className="space-y-3">
                <div className="space-y-1.5">
                  ...
                  <div className="relative">
                    ...
                    <Input placeholder="0.00" className="pl-7" value={form.price} onChange={(e) => set("price", e.target.value)} />
                  </div>
                </div>
                <div className="space-y-1.5">
                  <Label>Currency</Label>
                  <Input placeholder="USD" value={form.currency} onChange={(e) => set("currency", e.target.value)} />
                </div>
                <div className="space-y-1.5">
                ...
                <div className="space-y-1.5">
                  <Label>Effective To <span className="text-muted-foreground font-normal ml-1 text-xs">(optional)</span></Label>
